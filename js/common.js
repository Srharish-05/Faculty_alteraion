...
document.addEventListener('DOMContentLoaded', async () => {
    // 1. Session Check - ONLY if user has a valid token
    try {
        const token = localStorage.getItem('facultySync_token');
        
        // If no token, user is not logged in - skip session checks (they're on login page)
        if (!token) {
            return;
        }
...
function updateUserUI(user) {
    ...
    // Use textContent for user name (safe from XSS)
    nameEls.forEach(el => {
    ...
    if (avatarEls.length > 0) {
        avatarEls.forEach(el => {
            ...
            if (user.photo && isValidImageUrl(user.photo)) {
                // Create img element safely without innerHTML
    ...
    // Use textContent for role (safe from XSS)
    roleEls.forEach(el => {
        if (el) el.textContent = user.desig || 'Faculty';
    });
}
...
function isValidImageUrl(url) {
    try {
        ...
        return /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(pathname) || url.startsWith('data:image/');
    } catch (e) {
        return false;
    }
}
